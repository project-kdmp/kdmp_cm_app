...
| `LocaleDataException: Locale data has not been initialized` | The app gets locale data from `MaterialApp`'s localization delegates; unit tests do not. Call `initializeDateFormatting('<locale>')` in `setUpAll` (import `package:intl/date_symbol_data_local.dart`) |
...
- [ ] Date/number locale initialised if the code formats with a non-default locale
      (`initializeDateFormatting('ko_KR')` in `setUpAll`)
