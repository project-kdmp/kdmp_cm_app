...
class _PlaceSearchScreenState extends State<PlaceSearchScreen> with SingleTickerProviderStateMixin {
  ...
  void initScrollController() {
    _scrollController.addListener(() {
      if (_scrollController.position.maxScrollExtent == _scrollController.position.pixels) {
        if (!_placeSearchViewModel.isRecentListValid) {
          /// 검색 리스트 조회
  ...
  void initData() async {
    ...
    _placeSearchViewModel.clientId = dotenv.get(AppConstants.NAVER_CLIENT_ID);
    _placeSearchViewModel.clientSecret = dotenv.get(AppConstants.NAVER_CLIENT_SECRET);
    _placeSearchViewModel.jusoApiKey = dotenv.get(AppConstants.KAKAO_REST_API_KEY);
  ...
  Future<void> setMapData({required String addressRoad, required String addressJibun, required String place}) async {
    ...
    final result = await _placeSearchViewModel.getAddressInfo(address: addressRoad);
    if (result is Success) {
      ...
      final mapData = MapData(
        ...
        // TODO: 서버에 도로명, 지번 둘다 저장할 경우 addressJibun
        addressJibun: "",
        place: place,
      ...
      /// 검색된 주소로 장소 정보 검색 (지번)
      final result = await _placeSearchViewModel.getAddressInfo(address: addressJibun);
      if (result is Success) {
        final addressInfo = result.geocodingResponse.addresses![0];
        String sido = "";
        String sigugun = "";
        ...
        final mapData = MapData(
          latLng: NLatLng(double.parse(addressInfo.y), double.parse(addressInfo.x)),
          // TODO: 서버에 도로명, 지번 둘다 저장할 경우 addressRoad
          addressRoad: "",
