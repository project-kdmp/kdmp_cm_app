...
import 'package:kdmp_cm_app/presentation/viewmodel/payment/payment_password_viewmodel.dart';

/// 결제 비밀번호 입력 화면
class PaymentPasswordScreen extends StatefulWidget {
  const PaymentPasswordScreen({Key? key}) : super(key: key);

  static const String routeName = "payment_password";
...
class _PaymentPasswordScreenState extends State<PaymentPasswordScreen> {
  late final PaymentPasswordViewModel _paymentPasswordViewModel;
  ...
  /// Create
  void initViewModel() {
    _paymentPasswordViewModel = PaymentPasswordViewModel(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: BaseAppBar(
        appBar: AppBar(),
        title: StringPaymentPassword.inputTitle,
      ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  ValueListenableBuilder<String>(
                    valueListenable: _paymentPasswordViewModel.messageNotifier,
                  ...
                  /// 입력된 결제 비밀번호
                  ValueListenableBuilder<String>(
                    valueListenable: _paymentPasswordViewModel.inputPasswordNotifier,
                    builder: (context, value, child) {
                    ...
                    children: [
                      ...
                      Flexible(
                        ...
                        child: GestureDetector(
                          ...
                          onTap: () {
                            _paymentPasswordViewModel.removePasswordLastChar();
                          },
                        ),
                      ),
                    ],
  ...
  _addPasswordChar(int value) async {
    await _paymentPasswordViewModel.addPasswordChar(value);
    if (_paymentPasswordViewModel.inputPassword.length == 6) {
      /// 결제 비밀번호 비교
      final result = await _paymentPasswordViewModel.checkPassword();
      if (result == true) {
        /// 화면 닫기, 비밀번호 일치 여부 전달
        context.pop(result);
      }
